...
const statusColors = {
  optimal: 'fill-emerald-500/5 stroke-emerald-500/40 hover:fill-emerald-500/10',
  warning: 'fill-amber-500/10 stroke-amber-500/50 hover:fill-amber-500/15',
  critical: 'fill-rose-500/10 stroke-rose-500/50 hover:fill-rose-500/15',
  inactive: 'fill-slate-100 stroke-slate-200 hover:fill-slate-200',
};
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <rect
          ...
          height="200"
          rx="6"
          className="fill-slate-50 stroke-slate-200 stroke-[1.5]"
        />
        <line x1="340" y1="300" x2="460" y2="300" className="stroke-slate-200 stroke-[1.5]" />
        <circle cx="400" cy="300" r="15" className="fill-none stroke-slate-200 stroke-[1.5]" />
      ...
      <AnimatePresence>
        {hoveredZone && (
          <motion.div
            ...
            className="z-30 pointer-events-none bg-slate-900 border border-slate-950 px-3 py-2 rounded-xl shadow-xl flex flex-col text-[10px] space-y-0.5 text-white"
          >
            <span className="font-semibold">{hoveredZone.name}</span>
            <span className={`capitalize font-bold ${statusText[hoveredZone.status]}`}>
              Status: {hoveredZone.status}
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedZone && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-6 right-6 left-6 md:left-auto md:w-80 bg-white/95 border border-slate-200 p-5 rounded-2xl backdrop-blur-xl shadow-2xl z-20 flex flex-col"
          >
